<template>
  <div class="chain">
    <ComSelect
      ...
      @change="changeChain"
    />
    <div class="chain__setting">
      <div class="g-clickable" @click="refresh">
        <svg
          id="arrow_loading"
      ...
      <div class="g-clickable" @click="changeTheme">
        <i class="iconfont icon-Light dark" v-if="theme === 'dark'"></i>
        <i class="iconfont icon-dark light" v-else></i>
      </div>
      <div class="g-clickable" @click="$emit('setting')">
...
export default {
  ...
  data() {
    return {
      chainCode: this.chain,
      timeout: null,
    };
  },
  computed: {
    options() {
      const temp = this.chainList.map((item) => {
  ...
  mounted() {
    const animate = document.getElementById("animate_loading");
    animate.addEventListener("repeatEvent", () => {
  ...
  methods: {
    ...
    changeChain(chainCode) {
      const _chain = this.chainList.filter((item) => {
        return item.chain === chainCode;
      });
      const chain = _chain && _chain[0];
      const { code } = chain || {};
      if (this.chain === code) return;

      this.$emit("changeChain", chain, false);
    },
    changeTheme() {
